...
const test1a = document.querySelector("#test1-1"); // 버튼 자체를 얻어오는것

// ** 고전 이벤트 보델 작성법 **
// 요소.이벤트리스터 = 이벤트핸들러(수행될 함수)
...
// 고전 이벤트 모델 단점
// 아이디가 test1-3 요소를 얻어와 test1c 변수에 저장
...
// onclick에 저장된 값(이벤트 핼들러 함수)이 
// 덮어씌워지면서 이전코드(배경색 red)가 사라지는 문제 발생
// => 고전이벤트 모델의 단점 해결을 위해 "표준 이벤트 모델 형식" 많이 사용
...
// ** 표준 이벤트 모델 작성법 **
// 요소.addEverntListener("이벤트 종류", 이벤트핸들러(수행할 기능 함수));

test2.addEventListener("click", function(){
// 인라인 이벤트나 고전이벤트와 달리 이벤트 종류 쓸떄 on 안쓰고 그냥 종류만 쓴다!
...
box3.addEventListener("click", function(e){
  ...
  //alert(`배경색 : ${this.style.backgroundColor}`); // 방법2
  
  
/* Test
style 속성 이용하면 display 등 거이 모든 css 할 수 있다.
*/
})
