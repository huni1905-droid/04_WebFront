...
function print1(a, b){ // 함수 선언(매개변수 a, b)
  // 함수 정의
...
btn1.addEventListener("click", function(){
  ...
  let result; // 입력된 값이 0 / 양수 / 음수인지 판별해서 저장
  if(Number(value) === 0) result = "0";
        // ((value) == 0)로 써도 된다
  else if(Number(value) > 0) result = "양수";
  else result = "음수";    
  
  // "123은 양수입니다." 출력 print1() 함수 호출하여 사용
  print1(value, result); // 함수 호출(전달인자 value, result)
...
document.querySelector("#btn2a").addEventListener("click", function(){
  ...
  arrayTest(arr, x);//전달인자로 arr 전달
  
  console.log("arr[2] : ", arr[2]);
  // 왜 딸기가 아니라 멜론일까?
  // 배열은 참조형 -> arr에 저장된 데이터가 아니라 주소가 넘어간다
  ...
  // JS 원시타입 종류
  // string(Java랑 차이!) / number / boolean / undefined / null ..
  // 원시타입 : 객체가 아니라, 값 자체를 저장(원본 불변)
  
  // JS 객체타입 종류
  // Object, Array, Function..
  // 참조형(객체타입) : 메모리에 저장된 주소를 참조함
  // (복사본 변경되면, 원본도 변함)
})
...
document.querySelector("#btn2c").addEventListener("click", function(){
  ...
  // 함수도 변수에 저장 가능(변수명 == 함수명)(익명함수는 이런식으로 호출 가능) 
