// console.log(값);
// 브라우저(개발자도구) 콘솔창에 괄호() 내부의 값을 출력
console.log(1234); // number 타입의 숫자는 콘솔창에 약간 파란색으로 나온다
...
console.log("number1 : ", number1); //문자열(앞) number1 : / 숫자타입(뒤) number1
...
// const test; 상수는 선언과 동시에 초기화 반드시 해야한다!
//             (상수는 선언만 할 수 없다)
...
console.log(foo1); // 전역변수이기 때문에 (블록과 관계없이) 접근 가능
console.log(foo2); // 전역변수이기 때문에 (블록과 관계없이) 접근 가능
// console.log(num1);
// console.log(num2);
// -> 둘 다 블록레벨 스코프이기 때문에 블록 밖에서 접근할 수 없음

// if, for, while, switch, try {} emd -> 블록
...
function example(){ // 이렇게 함루 형태로 만들어진 
// 코드 블록은 함수레벨 scope
...
example();// examle()이란 함수를 호출까지 해줘야 한다.
...
function examle2() { // 함수 블록(함수레벨 scope)

  if(true) { // 일반 블록(블록레벨 scope) -> var는 무시하기 때문에
    var x = 10;
  }
...
console.log(a); // undefined : 선언만 되고 값이 없을 때
...
const gender = 'M'; // 홑따옴표 & 한글자만 작성해도 문자열
                    // (string O / char X(JS에는 char형 없다))
...
const numbers = [10, 20, 30]; // object 타입

console.log("numbers : ", numbers, typeof numbers);

console.log("numbers 배열 값 중 0번째 : ", 
    numbers[0], 
    typeof numbers[0]); // Number 타입

console.log("numbers 배열 값 중 1번째 : ", 
    numbers[1], 
    typeof numbers[1]); // Number 타입

console.log("numbers 배열 값 중 2번째 : ", 
    numbers[2], 
    typeof numbers[2]); // Number 타입
...
// 크기가 고정되지 않고 필요에 따라 자동으로 늘어나거나 즐어든다.


// 2) JS 객체
// 값을 K:V (Map) 형식으로 여러개 저장하는 형식
// -> { K:V, K:V, K:V, ...}
...
// key 쓸때 문자열로 "" 나 '' 로 작성하지 않으면 에러 발생
