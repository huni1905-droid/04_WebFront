...
function test(){
  const password =prompt("비밀버호를 입력하세요");

  console.log(password);

  // 확인 버튼 클릭 -> 입력한 값이 문자열로 저장
  // 취소 버튼 클릭 -> 콘솔창 : null
...
// 변수 선언
  ...
  const pw = '1234';  // 비밀번호 지정 변수(1234)

  output.innerText = balance; // 초기 금액  10000원 화면 출력
...
// 입금 함수
  ...
  function withdrawal(){
    if(amount.value.length == 0){
      ...
      const password = prompt("비밀버호를 입력하세요");
      // 사용자가 입력한 값 또는 null 값을 받아줄 password 지정
      ...
      if(password == null){ // 취소버튼 누름
        ...
        if(password != pw){ // 비밀번호가 불일치한 경우
          alert("비밀번호가 일치하지 않습니다");
          
        }else{ // 일치하는 경우

          const money = Number(amount.value); // money : 출금하려는 금액
